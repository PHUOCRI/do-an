...
function Iphone(props) {
    ...
    useEffect(() => {
        ...
        async function FetchApi(){
            try {
                setLoading(true)
                console.log('Đang gọi API iPhone...')
                ...
                if (validProducts.length > 0) {
                    ...
                    setHotIphone(normalizedProducts)
                    setError(null)
                } else {
                    setError('Không tìm thấy sản phẩm iPhone hợp lệ.')
                    setHotIphone([])
                }
            } catch (error) {
                if (!isMounted) return
                
                console.error('Lỗi khi tải dữ liệu iPhone:', error)
    ...
    const processedProducts = React.useMemo(() => {
        if (!Array.isArray(hotIphone) || hotIphone.length === 0) {
            return []
        }
        return handlePercentDiscount(hotIphone)
    }, [hotIphone])
